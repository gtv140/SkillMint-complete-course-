...
<head>
    ...
    <style>
        ...
        .trust-gradient { background: linear-gradient(135deg, #1e3a8a 0%, #1e40af 100%); }
    </style>
</head>
<body class="h-screen flex flex-col">
    ...
    <section id="auth-ui" class="fixed inset-0 z-[1000] bg-[#000103] flex items-center justify-center p-8 text-center">
        <div class="w-full max-w-sm">
            ...
            <div class="glass p-10 rounded-[3.5rem] border-t-2 border-blue-600 shadow-2xl">
                ...
                <input type="text" id="ref-by" placeholder="Referral ID (Optional)" class="w-full bg-white/5 p-4 rounded-xl border border-white/5 outline-none text-center text-[10px] mb-6 font-bold">
    ...
    <main id="app-ui" class="hidden flex-1 overflow-y-auto pb-32">
        <div id="p-home" class="page active-page p-6">
            <div class="trust-gradient p-10 rounded-[3rem] mb-6 shadow-2xl relative overflow-hidden">
                <div class="absolute top-0 right-0 p-4 opacity-10 text-6xl">🔒</div>
                <p class="text-[9px] text-blue-200 font-extrabold mb-1 uppercase tracking-widest">Total Asset Value</p>
                <h2 class="text-5xl font-black tracking-tighter" id="v-bal">₨ 0</h2>
                <div id="countdown-display" class="mt-4 text-[9px] font-black text-yellow-300 uppercase italic">Syncing Protocol...</div>
                <div class="mt-6 flex gap-3">
                    <span class="text-[8px] bg-white/10 text-white px-4 py-2 rounded-full font-bold border border-white/20 uppercase">Profit: <span id="v-profit">₨ 0</span></span>
                    <span id="tier-tag" class="text-[8px] bg-white/10 text-white px-4 py-2 rounded-full font-bold uppercase italic border border-white/20">Inactive</span>
                </div>
            </div>
        ...
        <div id="p-more" class="page p-6 space-y-6 pb-12">
            <div class="glass p-8 rounded-[3rem] border-t-4 border-blue-500">
                <h3 class="text-blue-500 font-black text-[11px] uppercase mb-2 text-center italic">My Referral ID</h3>
                ...
                <button onclick="copyRef()" class="w-full bg-blue-600/20 text-blue-400 border border-blue-500/30 py-3 rounded-2xl font-black text-[9px] uppercase mt-2">Copy Invite Link</button>
            </div>

            <div class="glass p-8 rounded-[3rem] border-l-4 border-blue-600">
                <h3 class="text-blue-500 font-black text-[11px] uppercase mb-4 italic">📜 Privacy & Terms</h3>
                <div class="text-[8px] text-gray-400 font-bold uppercase leading-relaxed space-y-3">
                    <p class="border-b border-white/5 pb-2"><span class="text-blue-400">Protocol:</span> All transactions are encrypted via MintCrest Multi-Sig protocol.</p>
                    <p class="border-b border-white/5 pb-2"><span class="text-blue-400">Withdrawals:</span> Payouts are manually verified and processed within 2-24 hours.</p>
                    <p class="border-b border-white/5 pb-2"><span class="text-blue-400">Anti-Fraud:</span> One account per device. Multi-accounting will result in permanent ban.</p>
                    <p><span class="text-blue-400">Yield:</span> Daily profit is credited exactly 24 hours after plan activation.</p>
                </div>
            </div>
    ...
    <script>
        ...
        async function login() {
            ...
            renderPlans(); 
            document.getElementById('my-ref-id').innerText = name;
        ...
        function renderPlans() {
            ...
            plans.forEach(p => {
                ...
                const totalReturn = ((p.p * p.r / 100) * p.d).toLocaleString();
                list.innerHTML += `<div onclick="buy(${p.p}, ${p.r}, '${p.n}')" class="glass p-5 rounded-[2.5rem] flex justify-between items-center active:scale-95 cursor-pointer mb-2" style="${style}"><div><h4 class="font-black text-[10px] uppercase ${p.s ? 'text-blue-400' : 'text-gray-400'}">${p.n}</h4><p class="text-[8px] text-green-400 font-bold uppercase">${p.r}% Daily Yield</p><p class="text-[7px] text-gray-500 font-bold uppercase italic mt-1">⏳ ${p.d} Days Contract</p></div><div class="text-right"><p class="font-black text-[12px]">₨ ${p.p.toLocaleString()}</p><p class="text-[6px] opacity-30 font-bold uppercase italic">Total: ₨ ${totalReturn}</p></div></div>`;
            });
        }
        ...
        async function applyPromoRequest() {
            ...
            alert("Promo Submitted!"); changePage('activity');
        }
        ...
        async function syncAdmin() {
            db.collection("requests").where("status", "==", "pending").onSnapshot(snap => {
                ...
                snap.forEach(doc => {
                    const d = doc.data(); 
                    list.innerHTML += `<div class="glass p-5 rounded-3xl mb-4"><div class="flex justify-between items-center text-[9px] font-black uppercase"><div>USER: ${d.user}<br>₨ ${d.amount} (${d.type})</div><button onclick="handle('${doc.id}','${d.user}',${d.amount},'approved','${d.type}')" class="bg-green-600 px-4 py-2 rounded-xl">YES</button></div></div>`;
                });
            });
        }
