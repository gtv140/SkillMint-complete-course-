...
<body class="h-screen flex flex-col">
    ...
    <section id="auth-ui" class="fixed inset-0 z-[1000] bg-[#010409] flex flex-col items-center justify-center p-8 text-center">
        ...
        <div class="glass p-10 rounded-[3.5rem] w-full max-w-sm neon-border">
            ...
            <button onclick="login()" class="w-full grad-blue py-5 rounded-2xl font-black text-[10px] uppercase text-white shadow-xl">Unlock Vault</button>
        </div>
    </section>
    ...
    <main id="app-ui" class="hidden flex-1 overflow-y-auto pb-32">
        <div id="p-home" class="page active-page p-6">
            ...
            <button onclick="requestBonus()" class="w-full grad-gold p-6 rounded-[2.5rem] mb-6 flex justify-between items-center active:scale-95">
                <div class="text-left"><h4 class="text-[11px] font-black uppercase text-white">Daily Bonus Claim</h4></div><span class="text-2xl">🎁</span>
            </button>
        </div>
        ...
        <div id="p-wallet" class="page p-6">
            <div class="glass p-10 rounded-[4rem] text-center border-t-8 border-blue-600 shadow-2xl">
                ...
                <input type="text" id="dep-trx" placeholder="TID / Hash Number" class="w-full bg-white/5 p-5 rounded-2xl mb-6 text-center font-bold border border-white/10 uppercase">
    ...
    <div id="admin-panel" class="fixed inset-0 bg-[#000814] z-[5000] hidden overflow-y-auto">
        <header class="p-8 border-b border-white/10 flex justify-between items-center sticky top-0 bg-[#000814]">
            <h2 class="text-xl font-black text-blue-500 uppercase italic">Command Node</h2>
            <button onclick="closeAdmin()" class="bg-red-600 px-6 py-2 rounded-xl text-[9px] font-black uppercase text-white">Exit</button>
        </header>
        <div id="adm-sec-requests" class="p-6 space-y-4"></div>
    </div>

    <nav id="bottom-nav" class="hidden glass border-t border-white/5 p-8 flex justify-around items-center fixed bottom-0 w-full z-[200] rounded-t-[4rem]">
        ...
        const plans = [{ n: "Micro-Elite I", p: 200, r: 3 }, { n: "Bronze-S", p: 1000, r: 3.5 }, { n: "Silver-X", p: 5000, r: 4.5 }, { n: "Gold-X", p: 10000, r: 5.5 }, { n: "Diamond-Master", p: 50000, r: 8 }];
        ...
        async function submitDeposit() {
            ...
            if(!a || !t || !file) return alert("Sweetie, upload proof also!");
            ...
            try {
                ...
                alert("Deposit Logged with Proof! 😘"); location.reload();
            } catch (e) { alert("Error uploading!"); btn.innerText = "Verify Capital"; btn.disabled = false; }
        }
        ...
        async function submitWithdraw() {
            ...
            alert("Authorized!"); location.reload();
        }
            ...
            db.collection("requests").where("status", "==", "pending").onSnapshot(snap => {
                ...
                snap.forEach(doc => { const d = doc.data(); 
                    list.innerHTML += `<div class="glass p-6 rounded-3xl mb-4 border border-white/10">
                        ...
                        ${d.proof ? `<a href="${d.proof}" target="_blank" class="block bg-amber-600 text-center py-2 rounded-xl text-[9px] font-black mb-3 uppercase text-white">View Screenshot</a>` : ''}
