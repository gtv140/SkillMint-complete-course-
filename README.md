...
<body class="h-screen flex flex-col">
    ...
    <main id="app-ui" class="hidden flex-1 overflow-y-auto pb-32">
        ...
        <div id="p-more" class="page p-6 space-y-6 pb-12">
            <div class="glass p-8 rounded-[3rem] border-t-4 border-yellow-500">
                ...
                <button onclick="applyPromoRequest()" class="w-full bg-yellow-600 py-4 rounded-2xl font-black text-[10px] uppercase shadow-lg">Submit Code</button>
            </div>
            ...
            <div class="glass p-8 rounded-[3rem] text-[7px] text-gray-500 font-bold uppercase leading-relaxed">
                <h4 class="text-blue-500 mb-3 text-[9px] text-center">📜 Official Terms</h4>
                <p class="mb-2">• Withdrawals processed in 2-24 hours.</p>
                <p>• Daily yield credited every 24 hours.</p>
            </div>
            <div class="glass p-8 rounded-[3rem]">
                ...
                <textarea id="support-msg" placeholder="Describe your issue..." class="w-full bg-white/5 p-5 rounded-2xl text-[10px] border border-white/10 outline-none mb-4 h-24 font-bold"></textarea>
        ...
        <div id="p-wallet" class="page p-6">
            <div class="glass p-10 rounded-[3.5rem] text-center border-t-8 border-blue-600 shadow-2xl">
                ...
                <div class="space-y-3 mb-10 text-[10px] font-black text-left">
                    ...
                    <div class="glass p-5 rounded-2xl flex flex-col bg-yellow-500/5"><span class="mb-1 text-yellow-500">BINANCE (USDT - TRC20)</span><span class="text-[8px] text-white break-all">TTSxm4pBK26RB4vXaa3Uo3hqGa5HdhxBDR</span></div>
                </div>
                
                <select id="dep-method" class="w-full bg-white/5 p-5 rounded-2xl mb-3 font-bold outline-none border border-white/5 text-[10px]">
                    <option value="JazzCash/SadaPay">JazzCash / SadaPay</option>
                    <option value="EasyPaisa">EasyPaisa</option>
                    <option value="Binance">Binance (USDT)</option>
                </select>
                <input type="number" id="dep-amount" placeholder="Amount (PKR/USDT)" class="w-full bg-white/5 p-5 rounded-2xl mb-3 text-center font-bold outline-none border border-white/5">
        ...
        <div id="p-withdraw" class="page p-6">
            <div class="glass p-10 rounded-[3.5rem] border-t-8 border-red-600 text-center shadow-2xl">
                ...
                <input type="text" id="wd-acc" placeholder="Account Details (Name, Number, Method)" class="w-full bg-white/5 p-5 rounded-2xl mb-8 text-center text-[9px] font-bold outline-none border border-white/5">
    ...
    <div id="admin-panel" class="fixed inset-0 bg-[#000103] z-[5000] p-6 hidden overflow-y-auto">
        ...
        <div id="adm-sec-broadcast" class="adm-tab hidden glass p-8 rounded-[3rem]">
            <textarea id="bc-msg" placeholder="Write Ticker News..." class="w-full bg-white/5 p-5 rounded-2xl text-[10px] border border-white/10 outline-none mb-4 h-24 font-bold"></textarea>
    ...
    <script>
        ...
        function startSync(name) {
            ...
            db.collection("requests").where("user", "==", name).onSnapshot(snap => {
                const list = document.getElementById('user-history'); list.innerHTML = snap.empty ? '<p class="text-center opacity-20 py-10 text-[9px] uppercase font-black italic">Ledger Empty</p>' : '';
                let items = []; snap.forEach(doc => items.push(doc.data())); items.sort((a,b) => b.time - a.time);
                items.forEach(d => { 
                    const clr = d.type.includes('dep') ? 'border-blue-500' : 'border-red-500'; 
                    list.innerHTML += `<div class="glass p-5 rounded-3xl flex justify-between items-center border-l-4 ${clr} mb-2 text-[8px] font-black uppercase"><div>${d.type}<br><span class="opacity-30 text-[6px]">${new Date(d.time).toLocaleString()}</span></div><div class="text-right">₨ ${d.amount}<br><span class="badge-${d.status}">${d.status}</span></div></div>`; 
                });
            });
        }
        ...
        function renderPlans() {
            ...
            plans.forEach(p => {
                const style = p.s ? 'border: 1px solid rgba(59, 130, 246, 0.3) !important;' : '';
                list.innerHTML += `<div onclick="buy(${p.p}, ${p.r}, '${p.n}')" class="glass p-5 rounded-[2.5rem] flex justify-between items-center active:scale-95 cursor-pointer mb-2" style="${style}"><div><h4 class="font-black text-[10px] uppercase">${p.n}</h4><p class="text-[8px] text-green-400 font-bold uppercase">${p.r}% Yield</p></div><div class="text-right"><p class="font-black text-[12px]">₨ ${p.p.toLocaleString()}</p></div></div>`;
            });
        }
        ...
        function updateCountdown() { if (user && user.activeTier > 0) { const d = (user.lastReqTime + 86400000) - Date.now(); if(d>0) { const h = Math.floor(d/3600000); const m = Math.floor((d%3600000)/60000); document.getElementById('countdown-display').innerText = `YIELD IN: ${h}H ${m}M`; } else { document.getElementById('countdown-display').innerText = "READY"; } } else { document.getElementById('countdown-display').innerText = "IDLE"; } }
        ...
        async function syncAdmin() {
            db.collection("requests").where("status", "==", "pending").onSnapshot(snap => {
                ...
                snap.forEach(doc => { const d = doc.data(); 
                    ...
                    list.innerHTML += `<div class="glass p-4 rounded-xl flex flex-col text-[8px] font-black uppercase mb-2">
                        <div class="flex justify-between"><span>User: ${d.user}</span><span>Rs ${d.amount}</span></div>
                        <div class="mt-1 text-blue-400">Type: ${d.type}</div>
                        ${d.tid ? `<div class="mt-1 text-green-400">TID: ${d.tid}</div>` : ''}
                        ${d.method ? `<div class="mt-1">Method: ${d.method}</div>` : ''}
                        ${d.acc ? `<div class="mt-1 text-yellow-400">Acc: ${d.acc}</div>` : ''}
                        <div class="flex gap-2 mt-3">${pBtn}<button onclick="handle('${doc.id}','${d.user}',${d.amount},'approved','${d.type}')" class="bg-green-600 px-4 py-1 rounded">YES</button><button onclick="handle('${doc.id}','${d.user}',${d.amount},'rejected','${d.type}')" class="bg-red-600 px-4 py-1 rounded">NO</button></div>
                    </div>`;
                });
            });
            db.collection("users").onSnapshot(snap => { const list = document.getElementById('adm-sec-users'); list.innerHTML = ''; snap.forEach(doc => { const u = doc.data(); list.innerHTML += `<div class="glass p-3 rounded-xl flex justify-between items-center text-[8px] mb-2 uppercase"><span>${u.name}</span><span>Bal: ${u.balance}</span></div>`; }); });
        }
